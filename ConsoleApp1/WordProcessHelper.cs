...
namespace MarketingPlatform.Client
{
    ...
    public class WordProcessHelper
    {
        ...
        /// <param name="undock">是否移除 内容中的Html标签，默认是</param>
        /// <returns>相似度百分之几，完全匹配相似度为1</returns>
        public static decimal StringCompute(string strA, string strB)
        {
            StringCompute stringcompute = new StringCompute();
            stringcompute.SpeedyCompute(strA, strB);
            return stringcompute.ComputeResult.Rate;
        }
    }

    internal class StringCompute
    {
        #region 私有变量
        /// <summary>
        /// 字符串1
